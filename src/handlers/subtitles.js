  ...
  try {
    ...
    const downloadPromise = (async () => {
      if (fileId.startsWith('subdl_')) {
        ...
        const implementationType = config.subtitleProviders?.opensubtitles?.implementationType || 'v3';
        const wantsAuth = implementationType === 'auth';
        const missingCreds = wantsAuth && (!config.subtitleProviders?.opensubtitles?.username || !config.subtitleProviders?.opensubtitles?.password);
        if (missingCreds) {
